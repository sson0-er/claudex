...
## Inputs
- `docs/claudex/<slug>/03-design/` (read `README.md` first, then the interface files)
...
2. Write each task to `docs/claudex/<slug>/tasks/<nn>-<name>.md` using `${CLAUDE_PLUGIN_ROOT}/templates/task.md`. `<nn>` starts at 01; `<name>` uses `[a-z0-9-]`. Task References must cite design files by path, e.g. `docs/claudex/<slug>/03-design/interfaces/<component>.md` (add `#heading` only when a file has several relevant sections). An implementer must be able to open exactly the files a task lists and build the task.
